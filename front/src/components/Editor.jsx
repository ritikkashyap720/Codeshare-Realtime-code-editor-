...
function Editor({ socketRef, roomId, onInputChange, onOutputChange }) {
  ...
  useEffect(() => {
    if (language === "python") {
      ...
      if (localStorage.getItem("c++Code")) {
        setCode(localStorage.getItem("c++Code"))
      } else {
  ...
  const handleCodeCompilation = async () => {
    if (code !== " " && code !== "#write your python code here " && code !== "//write your c++ code here" && code !== "//write your java code in main class \nclass Main {\npublic static void main(String[] args) {\n\n}\n}") {
      setLoader(true);
      setOutput("")
      setExecutionTime("")
      ...
      if (response) {
        ...
        if (result.jobId) {
          ...
          pollInterval = setInterval(async () => {
            ...
            }else if(codeResult.status== 'invalid job id'){
              setLoader(false)
              setStatus("rejected")
              clearInterval(pollInterval,()=>{
                console.log("interval clear")
